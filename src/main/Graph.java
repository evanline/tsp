...
public class Graph extends JPanel {
	...
	protected void paintComponent(Graphics g) {
		...
		double xScale = ((double) getWidth() - 2 * BORDER_GAP) / (algoritme3.size() - 1);
		...
		for (int i = 0; i < algoritme3.size(); i++) {
			if (!(i == 0)) {
				int x1 = (int) (i * xScale + BORDER_GAP);
				int y1 = (int) ((MAX_SCORE - algoritme3.get(i)) * yScale + BORDER_GAP);
		...
		for (int i = 0; i < algoritme3.size() - 1; i++) {
			if (algoritme3.size() > 10 && algoritme3.size() < 100) {
				i++;
			} else if (algoritme3.size() > 100) {
				i = i + 9;
			}

			int x0 = (i + 1) * (getWidth() - BORDER_GAP * 2) / (algoritme3.size() - 1) + BORDER_GAP;
			int x1 = x0;
